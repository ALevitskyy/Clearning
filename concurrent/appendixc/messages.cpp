...
struct withdraw
{
  ...
  withdraw(std::string const &account_,
           unsigned amount_,
           messaging::sender atm_queue_) : account(account_), amount(amount_),
...
struct verify_pin
{
  ...
  std::string pin;
  mutable messaging::sender atm_queue;
  verify_pin(std::string const &acount_, std::string const &pin_,
             messaging::sender atm_queue_) : account(account_),
                                             pin(pin_), atm_queue(atm_queue_) {}
}
...
struct get_balance
{
  ...
  mutable messaging::sender atm_queue;
  get_balance(
      std::string const &account_, messaging::sender atm_queue_) : account(account_), atm_queue(atm_queue_) {}
};
struct balance
{
  unsigned amount;
